...
class LinkedList(object):
    ...
    def get_node_by_index(self, idx):
        ...
        i = 0
        while current is not None:
            nodes[i] = current  # Add key (index) with a value of current node to the nodes dictionary.
            current = current.next
            i += 1
            if nodes.get(idx):  # Error handling in case idx does not exist in dictionary.
                return nodes[idx]
